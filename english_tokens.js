...
NEWLINE = "NEWLINE";
article_words = ["a","an", "the", "these", "those", "any", "all", "some", "teh", "that", "every", "each", "this"];
...
true_words = ["True", "true", "yes", "ja"];// ok vs done!  si vs if, da vs dar, oui --
...
keywords = prepositions.plus(modifier_words).plus(be_words).plus(comparison_words).plus(fillers).plus(nill_words).plus(done_words).plus(auxiliary_verbs).plus(conjunctions).plus(type_keywords).plus(otherKeywords).plus(numbers).plus(operators).plus(type_names);
