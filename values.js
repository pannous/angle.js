...
function do_evaluate_property(x, y) {
	if (!x) return y;
	if (!y) return x;
	verbose("do_evaluate_property '" + x + "' in " + y)
	// verbose("do_evaluate_property '" + attr.toString() + "' in " + node.toString())
	the.result = null;
	if (y[x] instanceof Function)return do_call(x, y)
	if (x[y] instanceof Function)return do_call(y, x)
	if (y[x])return y[x]
	if (x[y])return x[y]
	if (x.in(["type", "class", "kind"])) return get_class(y)
	if (y.in(["type", "class", "kind"])) return get_class(x)
	if (y instanceof Array) return y.map(a => do_evaluate_property(x, a))
	if (x instanceof Array) return x.map(a => do_evaluate_property(a, y))
	if (x instanceof ast.AST) return todo("do_evaluate_property")
	try {
		the.result = do_call(y, x)
		return the.result;
	} catch (e) {
